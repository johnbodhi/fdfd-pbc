...
module someFunctions
	contains
		...
		if ( string1 .eq. 'cell_size_in_z_direction_dz' ) then
         ...
         if ( string1 .eq. 'periodic_kx' ) then
			read(string2,*) periodic_kx; 
         end if
         
        if ( string1 .eq. 'periodic_ky' ) then
			read(string2,*) periodic_ky; 
        end if
        
        if ( string1 .eq. 'mode' ) then
			read(string2,*) mode; 
        end if
        
        if ( string1 .eq. 'theta_inc' ) then
			read(string2,*) theta_inc; 
        end if
        
        if ( string1 .eq. 'phi_inc' ) then
			read(string2,*) phi_inc; 
        end if
...
subroutine setCoefficients
    ...
    Bex(1:nx,1,2:nz) = Bex(1:nx,1,2:nz) - &
                    (-Cexhz(1:nx,1,2:nz)*Bhz(1:nx,1,2:nz)+Cexhz(1:nx,1,2:nz)*Bhz(1:nx,nyp1,2:nz)*exp(j*ky*Py) &
                     +Cexhy(1:nx,1,2:nz)*Bhy(1:nx,1,2:nz)-Cexhy(1:nx,1,2:nz)*Bhy(1:nx,1,1:nz-1));  
    ...
    Bez(2:nx,nyp1,1:nz) = Bez(2:nx,1,1:nz)*exp(-j*ky*Py);    
    ...
    ! floquet phase corrections 
    
    do mi = 1,nx,1
        do mj = 1,ny,1                    
			pex(mi,mj,1) = exp( j * kx * ( mi + 0.5 ) * dx ) * exp( j * ky * ( mj - 1 ) * dy );
			pey(mi,mj,1) = exp( j * kx * ( mi - 1 ) * dx )   * exp( j * ky * ( mj + 0.5 ) * dy );
			pez(mi,mj,1) = exp( j * kx * ( mi - 1 ) * dx )   * exp( j * ky * ( mj - 1 ) * dy );
			phz(mi,mj,1) = exp( j * kx * ( mi + 0.5 ) * dx ) * exp( j * ky * ( mj + 0.5 ) * dy );     
        enddo   
    enddo    
    ...
    do mi = 1,nx,1
        do mj = 1,ny,1       
            do mk = 1,nz,1
				Bex(mi,mj,mk) = Bex(mi,mj,mk) * pex(mi,mj,1);
				Bey(mi,mj,mk) = Bey(mi,mj,mk) * pey(mi,mj,1);
				Bez(mi,mj,mk) = 0.5 * ( Bez(mi,mj,mk) + Bez(mi,mj,mk) ) * pez(mi,mj,1);
                
				Bhx(mi,mj,mk) = 0.5 * ( Bhx(mi,mj,mk) + Bhx(mi,mj,mk) ) * phx(mi,mj,1);
				Bhy(mi,mj,mk) = 0.5 * ( Bhy(mi,mj,mk) + Bhy(mi,mj,mk) ) * phy(mi,mj,1);
				Bhz(mi,mj,mk) = Bhz(mi,mj,mk) * phz(mi,mj,1);              
...
subroutine setIncidentField
	...
	forall (indx=1:Nx,indy=1:Ny,indz=1:Nz) Einc_y(indx,indy,indz) = exp(-j*ko*indz*dz); 
	forall (indx=1:Nx,indy=1:Ny,indz=1:Nz) Hinc_x(indx,indy,indz) = exp(-j*ko*(indz+0.5)*dz); 
	!Hinc_y = Hinc_y / nu_o;
	Hinc_x = Hinc_x / nu_o;  
...
subroutine saveCapturedFieldsToFile
    ...
    write(1,*) 'kz = ', kz 
    write(1,*) 'kh = ', kh 
    write(1,*) 'theta_inc = ', theta_inc
    write(1,*) 'phi_incident = ', phi_inc
	write(1,*) 'nu_o = ', nu_o
    write(1,*) 'dt = ', dt
    write(1,*) 'Px = ', Px
    write(1,*) 'Py = ', Py
...
!===========================================================================================
	...
	do mi = 1,Nx,1
		do mj = 1,Ny,1
            write(1,*) real(Escat_z(mi,mj,rp)), aimag(Escat_z(mi,mj,rp));
		end do
    end do 	
    write(1,'(" ]; ")')
	...
	do mi = 1,Nx,1
		do mj = 1,Ny,1
            write(1,*) real(Hscat_z(mi,mj,rp)), aimag(Hscat_z(mi,mj,rp));
		end do
    end do 	
    write(1,'(" ]; ")')
...
!======================================================================================
	...
	do mi = 1,Nx,1
		do mj = 1,Ny,1
            write(1,*) real(Escat_z(mi,mj,tp)), aimag(Escat_z(mi,mj,tp));
		end do
    end do   
    write(1,'(" ]; ")')
	...
	do mi = 1,Nx,1
		do mj = 1,Ny,1
            write(1,*) real(Hscat_z(mi,mj,tp)), aimag(Hscat_z(mi,mj,tp));
		end do
    end do   
    write(1,'(" ]; ")')    
...
subroutine matvec(n,x,y)
    ...
    tmpx(:,1,1:Nzm1) = Chxez(:,1,1:Nzm1)*(x(:,1,1:Nzm1,3)-x(:,nyp1,1:Nzm1,3)*exp(j*ky*Py)) &
            + Chxey(:,1,1:Nzm1)*(-x(:,1,2:Nz,2)+x(:,1,1:Nzm1,2));
    
    tmpx(:,nyp1,1:Nzm1) = tmpx(:,1,1:Nzm1)*exp(-j*ky*Py);
    
    tmpy(1,:,1:Nzm1) = Chyex(1,:,1:Nzm1)*(x(1,:,2:Nz,1)-x(1,:,1:Nzm1,1)) &
            + Chyez(1,:,1:Nzm1)*(-x(1,:,1:Nzm1,3)+x(1,:,1:Nzm1,3)*exp(j*kx*Px));
    
    tmpy(nxp1,:,1:Nzm1) = tmpy(1,:,1:Nzm1)*exp(-j*kx*Px);
    
    tmpz(1,1:Nym1,:) = Chzey(1,1:Nym1,:)*(x(1,1:Nym1,:,2)-x(nxp1,1:Nym1,:,2)*exp(j*kx*Px)) &
            + Chzex(1,1:Nym1,:)*(-x(1,2:Ny,:,1)+x(1,1:Nym1,:,1));
    
    tmpz(nxp1,1:Nym1,:) = tmpz(1,1:Nym1,:)*exp(-j*kx*Px);
    
    tmpz(1:Nxm1,1,:) = Chzey(1:Nxm1,1,:)*(x(2:Nx,1,:,2)-x(1:Nxm1,1,:,2)) &
            + Chzex(1:Nxm1,1,:)*(-x(1:Nxm1,1,:,1)+x(1:Nxm1,nyp1,:,1)*exp(j*ky*Py));
    
    tmpz(1:Nxm1,nyp1,:) = tmpz(1:Nxm1,1,:)*exp(-j*ky*Py);
     
    tmpz(1,1,:) = Chzey(1,1,:)*(x(1,1,:,2)-x(nxp1,1,:,2)*exp(j*kx*Px)) &
            + Chzex(1,1,:)*(-x(1,1,:,1)+x(1,nyp1,:,1)*exp(j*ky*Py));
    
    tmpz(nxp1,1,:) = tmpz(1,1,:)*exp(-j*kx*Px);
     
    tmpz(1,nyp1,:) = tmpz(1,1,:)*exp(-j*ky*Py);
      
    tmpz(nxp1,nyp1,:) = tmpz(1,1,:)*exp(-j*kx*Px)*exp(-j*ky*Py);   
    
    
	y(1:Nx,1,2:Nz,1) = x(1:Nx,1,2:Nz,1) &
		- (Cexhz(1:Nx,1,2:Nz)*(-tmpz(1:Nx,1,2:Nz )+tmpz(1:Nx,nyp1,2:Nz)*exp(j*ky*Py)) &
		+  Cexhy(1:Nx,1,2:Nz)*(tmpy(1:Nx,1,2:Nz)-tmpy(1:Nx,1,1:Nzm1)));
     
	y(1:Nx,nyp1,2:Nz,1) =  y(1:Nx,1,2:Nz,1)*exp(-j*ky*Py);
 
	y(1,1:Ny,2:Nz,2) = x(1,1:Ny,2:Nz,2) &
		- (Ceyhx(1,1:Ny,2:Nz)*(-tmpx(1,1:Ny,2:Nz)+tmpx(1,1:Ny,1:Nzm1)) &
		+  Ceyhz(1,1:Ny,2:Nz)*(tmpz(1,1:Ny,2:Nz)-tmpz(nxp1,1:Ny,2:Nz)*exp(j*kx*Px)));
    
	y(nxp1,1:Ny,2:Nz,2) = y(1,1:Ny,2:Nz,2)*exp(-j*kx*Px);
    
	y(1,2:Ny,1:Nz,3) = x(1,2:Ny,1:Nz,3) &
		- (Cezhy(1,2:Ny,1:Nz)*(-tmpy(1,2:Ny,1:Nz)+tmpy(nxp1,2:Ny,1:Nz)*exp(j*kx*Px)) &
		+  Cezhx(1,2:Ny,1:Nz)*(tmpx(1,2:Ny,1:Nz)-tmpx(1,1:Nym1,1:Nz)));
    
	y(nxp1,2:Ny,1:Nz,3) = y(1,2:Ny,1:Nz,3)*exp(-j*kx*Px);
    
	y(2:Nx,1,1:Nz,3) = x(2:Nx,1,1:Nz,3) &
		- (Cezhy(2:Nx,1,1:Nz)*(-tmpy(2:Nx,1,1:Nz)+tmpy(1:Nxm1,1,1:Nz)) &
		+  Cezhx(2:Nx,1,1:Nz)*(tmpx(2:Nx,1,1:Nz)-tmpx(2:Nx,nyp1,1:Nz)*exp(j*ky*Py)));
    
	y(2:Nx,nyp1,1:Nz,3) = y(2:Nx,1,1:Nz,3)*exp(-j*ky*Py);
     
	y(1,1,1:Nz,3) = x(1,1,1:Nz,3) &
		- (Cezhy(1,1,1:Nz)*(-tmpy(1,1,1:Nz)+tmpy(nxp1,1,1:Nz)*exp(j*kx*Px)) &
		+  Cezhx(1,1,1:Nz)*(tmpx(1,1,1:Nz)-tmpx(1,nyp1,1:Nz)*exp(j*ky*Py)));
    
    y(nxp1,1,1:Nz,3) = y(1,1,1:Nz,3)*exp(-j*kx*Px);
    
    y(1,nyp1,1:Nz,3) = y(1,1,1:Nz,3)*exp(-j*ky*Py);
    
    y(nxp1,nyp1,1:Nz,3) = y(1,1,1:Nz,3)*exp(-j*kx*Px)*exp(-j*ky*Py);
